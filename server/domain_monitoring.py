...
def urlscanio(domain):
    try:
        ...
        logging.info(f'Domain monitoring: {len(response_json)} results for {domain}')

        return response_json
    except requests.exceptions.RequestException as e:
        logging.error(f'Domain monitoring: Request error: {e}')
    except KeyError as e:
        logging.error(f'Domain monitoring: KeyError - \'results\' key not found in the response: {e}')
